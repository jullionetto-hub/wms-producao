...
function dataHoraLocal() {
  ...
  const partes = agora.toLocaleDateString('pt-BR',{timeZone:'America/Sao_Paulo',year:'numeric',month:'2-digit',day:'2-digit'}).split('/');
  const dataISO = `${partes[2]}-${partes[1]}-${partes[0]}`;
  const hora = agora.toLocaleTimeString('pt-BR',{timeZone:'America/Sao_Paulo',hour:'2-digit',minute:'2-digit',hour12:false});
...
// ── Helpers ───────────────────────────────────────────────────────────────────
function perfisPermitidos(u) {
  const ex = String(u.perfis_acesso||'').split(',').map(s=>s.trim()).filter(Boolean);
  return [...new Set([u.perfil,...ex])];
}
function hashSenha(s) { return crypto.createHash('sha256').update(s+'wms_salt_2026').digest('hex'); }

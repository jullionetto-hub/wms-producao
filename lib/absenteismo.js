...
function classificarDia(registros, horario, esperadoPorGrupo, diaSemana) {
  ...
  // Banco de horas do dia: horário real de saída (última marcação do dia)
  // menos o horário oficial de saída do turno. Ex: entrada 06:00, sairia
  // 15:00, saiu 15:30 → banco de +30min. Só compara a saída — não entra
  // atraso de entrada nem duração da pausa (esses já são penalizados à
  // parte como atraso de entrada/almoço/pausa).
  const saidaReal = registros[n - 1];
  if (oficial && oficial.saida) {
    out.banco_horas_min = paraMinutos(saidaReal) - paraMinutos(oficial.saida);
  } else if (medianas) {
    out.banco_horas_min = paraMinutos(saidaReal) - medianas[n - 1];
  } else {

...
function extrairDias(bloco) {
  ...
  const reLinha = new RegExp(
    `(\\d{2}\\/\\d{2}\\/\\d{4})\\s+(${DIAS})\\s+(\\d+)\\s+([\\s\\S]*?)(?=\\d{2}\\/\\d{2}\\/\\d{4}\\s+(?:${DIAS})|\\bTOTAL\\b)`,
    'g'
  );
  let m;
  while ((m = reLinha.exec(bloco))) {
    const [, dataStr, diaSemana, , resto] = m;
    const registros = [...resto.matchAll(/\b(\d{2}:\d{2})[oi]\b/g)].map(x => x[1]);
    const statusMatch = resto.match(new RegExp(`\\b(${STATUS_PALAVRAS.join('|')})\\b`));
    dias.push({

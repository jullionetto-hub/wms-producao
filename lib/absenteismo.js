...
const TURNOS_OFICIAIS = [
  ...
  { chave: 'tarde',     entrada: '13:00', almoco_min: 60, pausa_min: 15 },
  { chave: 'manh',      entrada: '06:00', almoco_min: 60, pausa_min: 15 }, // "manh" casa Manhã/Manha
];

function turnoOficial(horarioTexto) {
  const t = (horarioTexto || '').toLowerCase();
  return TURNOS_OFICIAIS.find(o => t.includes(o.chave)) || null;
}
...
function classificarDia(registros, horario, esperadoPorGrupo) {
  ...
  if (n < 2) return {};
  const oficial = turnoOficial(horario);

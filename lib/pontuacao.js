...
function calcularPesoCorredor(endereco) {
  ...
  const endFull = String(endereco).split(',')[0].trim().toUpperCase();
  // Endereços compostos: "N144/VERT-N08-CX21" ou "P200/VERT-Q04-CX24"
  // O separador vai ao endereço PRIMÁRIO (antes do '/').
  // O VERT é fallback na mesma passagem — não é deslocamento extra.
  // Só aplica peso VERT se o endereço inteiro for VERT (sem localização primária).
  const end = endFull.includes('/') ? endFull.split('/')[0].trim() : endFull;
...
// Custo de deslocamento por localização única: fator adicionado à quantidade
// para que pedidos com muitos produtos (locais) sejam mais pesados que pedidos
// com muitos itens de um único produto. Cada visita a uma posição = 20 "unidades".
const FATOR_LOCALIZACAO = 20;

function calcularPontuacaoPedido(itens) {
  if (!itens?.length) return 0;

  // Agrupa por endereço único (cada posição = uma visita ao armazém)
  const porLocal = {};
  for (const i of itens) {
    const end = String(i.endereco || 'X').split(',')[0].trim();
    porLocal[end] = (porLocal[end] || 0) + (parseInt(i.quantidade) || 1);
  }

  let score = 0;
  for (const [end, qty] of Object.entries(porLocal)) {
    const peso = calcularPesoCorredor(end);
    // custo = peso_corredor × (itens_no_local + deslocamento_fixo)
    score += peso * (qty + FATOR_LOCALIZACAO);
  }

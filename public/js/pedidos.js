...
async function carregarPedidos() {
  try {
    ...
    const usrId   = document.getElementById('filtro-ped-sep').value; // agora ├® o nome do separador
    ...
    // Filtra por usu├írio usando separador_nome (evita confus├úo entre usuario_id e separador_id)
    if (usrId) {
      // usrId agora ├® o nome do separador diretamente
    ...
    tbody.innerHTML = ps.map(p=>`<tr>
      ...
      <td style="font-size:11px;color:var(--text2);max-width:130px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap" title="${p.cliente||''}">${p.cliente||'ÔÇö'}</td>
      <td style="font-size:11px;font-weight:700;color:${String(p.transportadora||'').toUpperCase().includes('DRIVE')?'var(--red)':'var(--indigo)'}">${p.transportadora||'ÔÇö'}</td>
      <td style="font-size:11px;color:var(--amber);font-weight:600;white-space:nowrap">${p.aguardando_desde||'ÔÇö'}</td>
      <td style="font-size:12px;color:var(--text2)">${p.separador_nome||'ÔÇö'}</td>
      <td><span class="pill ${(p.status||'').replace(' ','-')}">${p.status}</span></td>
      <td style="font-weight:600;color:${(p.itens||0)>20?'var(--red)':(p.itens||0)>10?'var(--amber)':'var(--text)'}">${p.itens||'ÔÇö'}</td>
    </tr>`).join('');
  } catch(e) {}
}
...
async function atribuirSeparador(pid, sid) {
  ...
  try {
    ...
    toast('Usu├írio atribu├¡do!', 'sucesso');
...
async function carregarUsuarios() {
  try {
    ...
    if (!el) return;
    if (!users.length) {
      el.innerHTML = '<div style="color:var(--text3);text-align:center;padding:24px;font-size:13px">Nenhum usu├írio cadastrado</div>';
      return;
    }
    const countEl = document.getElementById('usr-count');
    if (countEl) countEl.textContent = `ÔÇó ${users.length} usu├írio(s)`;
    el.innerHTML = users.map(u => {
      ...
      const perfIcons = {supervisor:'­ƒæö',separador:'­ƒôª',repositor:'­ƒöº',checkout:'­ƒÅÀ´©Å'};
      return `<div class="usr-card ${u.status}">
        ...
        <div class="usr-info">
          ...
          <div class="usr-pills">
            ...
            <span class="usr-pill turno">ÔÅ░ ${u.turno||'ÔÇö'}</span>
        ...
        <div class="usr-actions">
          <button class="usr-btn ${u.status==='ativo'?'toggle-on':'toggle-off'}" title="${u.status==='ativo'?'Desativar':'Ativar'}"
            ...
            ${u.status==='ativo'?'ÔÅ©':'ÔûÂ'}
          </button>
          <button class="usr-btn del" title="Excluir" onclick="excluirUsuario(${u.id},'${u.nome}')">­ƒùæ</button>
        </div>
      </div>`;
    }).join('');
  } catch(e) {}
...
async function cadastrarUsuario() {
  ...
  if (senha.length < 6) { toast('Senha m├¡nimo 6 caracteres!','aviso'); return; }
  try {
    ...
    toast('Usu├írio cadastrado!','sucesso');
...
async function alterarStatusUsuario(id, novoStatus, nome, login, perfil, turno) {
  try {
    await fetch(`${API}/usuarios/${id}`, { credentials:'include', method:'PUT', headers:{'Content-Type':'application/json'}, body:JSON.stringify({nome,login,perfil,turno:turno||'Manh├ú',status:novoStatus}) });
    toast(`Usu├írio ${novoStatus==='ativo'?'ativado':'desativado'}!`,'sucesso');
    carregarUsuarios();
...
async function excluirUsuario(id, nome) {
  ...
  try {
    ...
    toast('Exclu├¡do!','sucesso'); carregarUsuarios();
  } catch(e) { toast('Erro!','erro'); }
}
...
function processarArquivoFile(file) {
  mostrarStatus('ÔÅ│ Lendo arquivo...','carregando');
  ...
  reader.onload = function(e) {
    try {
      ...
      // Mapeamento flex├¡vel: Pedido, Codigo, Descricao, Qtde, Endere├ºo
      ...
      if (!dados.length) { mostrarStatus('ÔØî Nenhuma linha encontrada!','erro'); return; }
      pedidosImportar = dados;
      const totalP = new Set(dadosUsar.map(d=>d.numero_pedido)).size;
      mostrarStatus(`Ô£à ${dados.length} linha(s) em ${totalP} pedido(s) ÔÇö clique Importar`,'sucesso');
      ...
      document.getElementById('txt-total-import').textContent = `${totalP} pedido(s) ÔÇó ${dados.length} itens`;
      document.getElementById('preview-importacao').style.display = 'block';
    } catch(err) { mostrarStatus(`ÔØî ${err.message}`,'erro'); }
  };
  reader.onerror = () => mostrarStatus('ÔØî Erro ao abrir arquivo!','erro');
 ...
 async function confirmarImportacao() {
  ...
  mostrarStatus('ÔÅ│ Importando...', 'carregando');




  // Agrupa por pedido antes de enviar ÔÇö garante que todos os itens ficam juntos
  const pedMapLocal = {};
  ...
  try {
    for (let i = 0; i < numeros.length; i += LOTE_PEDIDOS) {
      ...
      mostrarStatus(`ÔÅ│ Importando... ${progresso}% (${Math.min(i+LOTE_PEDIDOS,numeros.length)}/${numeros.length} pedidos)`, 'carregando');
      ...
      const data = await res.json();
      if (data.erro) { mostrarStatus(`ÔØî ${data.erro}`, 'erro'); return; }
    ...
    renderHistorico();
    mostrarStatus(`Ô£à ${totalImportados} pedido(s) importado(s)!${totalIgnorados > 0 ? ` ÔÜá´©Å ${totalIgnorados} j├í existiam.` : ''}`, 'sucesso');
    document.getElementById('preview-importacao').style.display = 'none';
    pedidosImportar = [];
    document.getElementById('input-arquivo').value = '';
    toast(`${totalImportados} pedidos na fila!`, 'sucesso');
  } catch(e) {
    mostrarStatus('ÔØî Erro na importa├º├úo!', 'erro');
  }
}
...
function renderHistorico() {
  ...
  if (!historicoImportacoes.length) { el.innerHTML = '<div style="color:var(--text3);font-size:11px;text-align:center;padding:14px">Nenhuma importa├º├úo</div>'; return; }
  el.innerHTML = historicoImportacoes.map(h=>`
    <div class="hist-item">
      <div><div style="color:var(--green);font-weight:700">Ô£à ${h.ok} pedido(s)</div>${h.erro>0?`<div style="color:var(--amber);font-size:10px">ÔÜá´©Å ${h.erro} j├í existiam</div>`:''}</div>
      <div style="color:var(--text3);font-size:10px">${h.data} ├ás ${h.hora}</div>
    </div>`).join('');
}
...
function limparHistorico() {
  ...
  renderHistorico(); toast('Hist├│rico limpo!','info');
}
  ...
  try {
    ...
    lista.innerHTML = rows.map(r => `
      <div style="display:flex;align-items:center;justify-content:space-between;padding:11px 14px;border-bottom:1px solid #FECACA;flex-wrap:wrap;gap:8px">
        <div>
          <div style="font-weight:700;color:var(--red);font-size:14px">Ôøö Pedido #${r.numero_pedido}</div>
          <div style="font-size:11px;color:var(--text3);margin-top:2px">
            ­ƒæñ ${r.separador_nome||'ÔÇö'} &nbsp;ÔÇó&nbsp; 
            Itens bloqueados: <b style="color:var(--text)">${r.codigos_bloqueados||'ÔÇö'}</b>
          </div>
        </div>
        <button class="btn btn-success btn-sm" onclick="desbloquearPedido(${r.id},'${r.numero_pedido}')">Ô£à Liberar Pedido</button>
      </div>`).join('');
  } catch(e) {}
}
...
async function desbloquearPedido(id, num) {
  if (!confirm(`Liberar pedido #${num}? Ele ser├í marcado como conclu├¡do.`)) return;
  try {
    await fetch(`${API}/pedidos/${id}/desbloquear`, { credentials:'include', method:'PUT' });
    toast(`Ô£à Pedido #${num} liberado!`,'sucesso');
...
function exportarExcel(tipo) {
  ...
  try {
    if (tipo === 'pedidos') {
      rows = [['N┬║ Pedido','Usu├írio','Status','Itens','Data','Hora']];
      ...
      rows = [['Separador','Hoje','M├¬s','Ano','Status']];
      ...
      rows = [['Repositor','Hoje','Repostos','N├úo Encontrados','Total']];
      ...
      rows = [['Caixa','N┬║ Pedido','Separador','Status','Hora']];
      ...
      rows = [['Caixa','N┬║ Pedido','Separador','Status','Data','Hora']];
    ...
    // Define largura das colunas automaticamente baseado no conte├║do
...
function exportarAvisosExcel() {
  try {
    ...
    const rows = [['C├│digo','Descri├º├úo','Endere├ºo','Pedido','Qtde','Status','Hora Aviso']];
    document.querySelectorAll('#lista-avisos .aviso-card').forEach(card => {
      ...
      const pedido = card.querySelector('.aviso-cod span')?.textContent?.replace('Pedido #','').trim() || 'ÔÇö';
      const desc   = card.querySelector('.aviso-desc')?.textContent?.trim() || 'ÔÇö';
      const det    = card.querySelector('.aviso-det')?.textContent?.trim() || '';
      const cls    = [...card.classList].find(c => ['pendente','reposto','nao_encontrado','protocolo'].includes(c)) || 'ÔÇö';
      const hora   = card.querySelector('[style*="hora_aviso"], [style*="hora_reposto"]')?.textContent?.trim() || 'ÔÇö';
    ...
    toast('Ô£à Excel exportado!','sucesso');
  } catch(e) { toast('Erro ao exportar!','erro'); }
}
...
async function confirmarPedido() {
  ...
  if (!num) { toast('Digite o n├║mero!','aviso'); return; }
...
async function carregarFila() {
  try {
    ...
    // S├│ mostra pedidos atribu├¡dos a mim, ordenados por itens ASC (menor primeiro)
    ...
    const html = !ordenadosFila.length
      ...
      : ordenadosFila.map(p => {
          return `<tr class="meu" onclick="selecionarPedidoFila('${p.numero_pedido}')" style="cursor:pointer">
            ...
            <td style="color:var(--green);font-weight:600">${p.itens||'ÔÇö'}</td>
            <td><span class="pill ${(p.status||'').replace(' ','-')}">${p.status}</span></td>
...
/* PEDIDOS PENDENTES DE REPOSI├ç├âO (separador) */
  ...
  try {
    ...
    if (!meus.length) {
      el.innerHTML = '<div style="background:#F0FDF4;border:1px solid #C6F6D5;border-radius:10px;padding:14px 16px;text-align:center;">' +
        '<div style="font-size:13px;font-weight:500;color:#15803D;">Nenhum item aguardando reposi├º├úo</div>' +
        '<div style="font-size:11px;color:#16A34A;margin-top:4px;">Todos os seus pedidos est├úo completos</div>' +
        '</div>';
      return;
    }
      ...
      html += `<div style="background:var(--surface);border:1px solid #FECACA;border-left:3px solid #DC2626;border-radius:0 10px 10px 0;margin-bottom:8px;overflow:hidden;">
        <div style="display:flex;align-items:center;justify-content:space-between;padding:10px 14px;background:#FFF8F8;">
          <div>
            ...
            <span style="font-size:10px;color:#94A3B8;margin-left:8px;">desde ${tempoMaisAntigo||'ÔÇö'}</span>
          </div>
          ...
          <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:8px;">
            <div style="flex:1;min-width:0;">
              <div style="display:flex;align-items:center;gap:6px;margin-bottom:2px;">
                <span style="font-family:'Space Mono',monospace;font-size:12px;font-weight:500;color:#0F172A;">${a.endereco||'ÔÇö'}</span>
              ...
              <div style="font-size:10px;color:#64748B;font-family:monospace;">${a.codigo||'ÔÇö'}</div>
              <div style="font-size:12px;color:#0F172A;line-height:1.3;margin-top:1px;">${a.descricao||'ÔÇö'}</div>
            </div>
            <div style="font-size:20px;font-weight:500;color:#0F172A;flex-shrink:0;">├ù${a.quantidade||1}</div>
          </div>
        </div>`;
      });

      // Bot├úo para ir ao pedido
...
function processarArquivoModalFile(file) {
  mostrarStatusModal('ÔÅ│ Lendo arquivo...','carregando');
  document.getElementById('modal-preview-importacao').style.display = 'none';
  const reader = new FileReader();
    ...
    try {
      ...
      if (transpSheet) {
        ...
        for (let i = 1; i < tRows.length; i++) {
          ...
          if (!num) continue;
          // Aba Transportadora: N┬║ pedido | Aguardando desde | Raz├úo social | Qtde | Tipo entrega
          ...
          if (r[iAg] !== undefined && r[iAg] !== null && r[iAg] !== '') {
            ...
            if (typeof raw === 'number') {
              ...
              // parte decimal = fra├º├úo do dia (0.5 = 12:00)
              // Converte para ms: (serial - 25569) * 86400000 ÔåÆ UTC
              // Ajusta para hor├írio de Bras├¡lia (UTC-3)
              try {
                const d = XLSX.SSF.parse_date_code(raw, {date1904: false});
                if (d && d.y > 2000) {
                  agVal = `${pad(d.d)}/${pad(d.m)}/${d.y} ${pad(d.H)}:${pad(d.M)}`;
                } else {
                  // Fallback: convers├úo manual
                  const ms = Math.round((raw - 25569) * 86400 * 1000);
                  const dt = new Date(ms);
                  // Ajusta UTC-3 (Bras├¡lia)
              ...
              // Date JS ÔÇö usar hor├írio local do browser
      ...
      // Filtra: s├│ importa pedidos que existem na aba Transportadora
      // (pedidos sem correspond├¬ncia s├úo de outros lotes/datas e devem ser ignorados)
      ...
      if (transpSheet && Object.keys(transpData).length > 0) {
        ...
        if (ignorados > 0) {
          mostrarStatusModal(`ÔÜá´©Å ${ignorados} pedido(s) da aba Itens n├úo encontrados na aba Transportadora foram ignorados.`, 'aviso');
      ...
      if (!dadosUsar.length) { mostrarStatusModal('ÔØî Nenhuma linha encontrada!','erro'); return; }
      pedidosImportarModal = dadosUsar;
      const totalP = new Set(dadosUsar.map(d=>d.numero_pedido)).size;
      mostrarStatusModal(`Ô£à ${dados.length} linha(s) em ${totalP} pedido(s)${transpSheet?' ÔÇö Transportadora OK':''}`, 'sucesso');
      document.getElementById('modal-tbody-prev').innerHTML =
        ...
        (dados.length>10?`<tr><td colspan="5" style="color:var(--text3);text-align:center;padding:8px">... +${dados.length-10} linhas</td></tr>`:'');
      document.getElementById('modal-txt-total-import').textContent = `${totalP} pedido(s) ÔÇó ${dados.length} itens${transpSheet?' ÔÇó Transportadora OK':''}`;
      document.getElementById('modal-preview-importacao').style.display = 'block';
    } catch(err) { mostrarStatusModal(`ÔØî ${err.message}`,'erro'); }
  };
  reader.onerror = () => mostrarStatusModal('ÔØî Erro ao abrir arquivo!','erro');
  reader.readAsArrayBuffer(file);
}
async function confirmarImportacaoModal() {
  if (!pedidosImportarModal.length) return;
  mostrarStatusModal('ÔÅ│ Importando...', 'carregando');
  const pedMapLocal = {};
  ...
  try {
    for (let i = 0; i < numeros.length; i += 20) {
      ...
      const linhasLote = [];
      loteNums.forEach(n => linhasLote.push(...pedMapLocal[n]));
      mostrarStatusModal(`ÔÅ│ Importando... ${Math.round(((i+loteNums.length)/numeros.length)*100)}%`, 'carregando');
      ...
      const data = await res.json();
      if (data.erro) { mostrarStatusModal(`ÔØî ${data.erro}`, 'erro'); return; }
      totalImportados += data.importados || 0;
      totalIgnorados  += data.ignorados  || 0;
    }
    ...
    localStorage.setItem('historico_importacoes', JSON.stringify(historicoImportacoes));
    renderHistoricoModal();
    mostrarStatusModal(`Ô£à ${totalImportados} pedido(s) importado(s)!${totalIgnorados>0?` ÔÜá´©Å ${totalIgnorados} j├í existiam.`:''}`, 'sucesso');
    document.getElementById('modal-preview-importacao').style.display = 'none';
    pedidosImportarModal = [];
    document.getElementById('modal-input-arquivo').value = '';
    toast(`${totalImportados} pedidos na fila!`, 'sucesso');
    carregarPedidos();
  } catch(e) { mostrarStatusModal('ÔØî Erro na importa├º├úo!', 'erro'); }
}
function renderHistoricoModal() {
  const el = document.getElementById('modal-hist-importacoes');
  if (!el) return;
  if (!historicoImportacoes.length) { el.innerHTML = '<div style="color:var(--text3);font-size:11px;text-align:center;padding:14px">Nenhuma importa├º├úo</div>'; return; }
  el.innerHTML = historicoImportacoes.map(h=>`<div class="hist-item"><div><div style="color:var(--green);font-weight:700">Ô£à ${h.ok} pedido(s)</div>${h.erro>0?`<div style="color:var(--amber);font-size:10px">ÔÜá´©Å ${h.erro} j├í existiam</div>`:''}</div><div style="color:var(--text3);font-size:10px">${h.data} ├ás ${h.hora}</div></div>`).join('');
}
...
async function abrirModalDistribuicao() {
  ...
  // Recalcula pontua├º├úo de pedidos antigos em background
...
function renderSepsDistribuicao(seps) {
  ...
  el.innerHTML = seps.map(s => {
    const turnoColor = {Manh├ú:'#f59e0b', Tarde:'#3b82f6', Noite:'#8b5cf6'}[s.turno] || 'var(--text3)';
    ...
    return `<label style="display:flex;align-items:center;gap:6px;padding:6px 10px;border:1.5px solid var(--border);border-radius:8px;background:var(--surface2);cursor:pointer;font-size:12px;font-weight:600">
      ...
      ­ƒæñ ${s.nome}${turnoLabel}
    </label>`;
  }).join('');
}

function filtrarTurnoDistribuicao(turno) {
  // Atualiza visual dos bot├Áes
  const map = {todos:'', manha:'Manh├ú', tarde:'Tarde', noite:'Noite'};
...
async function carregarPedidosDistribuicao() {
  try {
    ...
    el.innerHTML = `<div style="font-size:11px;color:var(--text3);margin-bottom:8px">${lista.length} de ${pedidos.length} pedido(s) ser├úo distribu├¡dos</div><div class="tabela-wrap" style="max-height:240px;overflow-y:auto"><table><thead><tr><th>PEDIDO</th><th>CLIENTE</th><th>HOR├üRIO</th><th>ITENS</th><th>PONTUA├ç├âO</th><th>STATUS</th></tr></thead><tbody>${lista.map(p=>`<tr><td style="font-weight:700;color:var(--text);font-family:'Space Mono',monospace;font-size:11px">${p.numero_pedido}</td><td style="font-size:11px;color:var(--text2);max-width:110px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${p.cliente||'ÔÇö'}</td><td style="font-size:11px;color:var(--amber);font-weight:600;white-space:nowrap">${(p.aguardando_desde||p.hora_pedido||'ÔÇö').replace('15/04/2026 ','').replace('16/04/2026 ','')}</td><td style="font-weight:600">${p.itens||0}</td><td><span style="font-family:'Space Mono',monospace;color:var(--indigo);font-weight:700">${p.pontuacao||'ÔÇö'}</span></td><td><span class="pill ${(p.status||'pendente')}">${p.status||'pendente'}</span></td></tr>`).join('')}</tbody></table></div>`;
  } catch(e) {}
}
async function calcularDistribuicao() {
  ...
  try {
    ...
    let html = '<div style="font-size:11px;font-weight:700;color:var(--accent);letter-spacing:1px;margin-bottom:10px">RESULTADO DA DISTRIBUI├ç├âO</div><div class="tabela-wrap"><table><thead><tr><th>SEPARADOR</th><th>PEDIDOS</th><th>PONTUA├ç├âO</th><th>LISTA</th></tr></thead><tbody>';
    data.plano.forEach(item => { html += `<tr><td style="font-weight:700;color:var(--text)">­ƒæñ ${item.separador_nome}</td><td style="color:var(--green);font-weight:700">${item.pedidos.length}</td><td><span style="font-family:'Space Mono',monospace;color:var(--indigo)">${item.pontuacao_total}</span></td><td style="font-size:11px;color:var(--text3)">${item.pedidos.join(', ')}</td></tr>`; });
  ...
  } catch(e) { toast('Erro ao calcular distribui├º├úo!', 'erro'); }
}
async function confirmarDistribuicao() {
  ...
  try {
    ...
    const data = await res.json();
    if (data.erro) { toast(data.erro, 'erro'); return; }
    toast(`Ô£à ${data.distribuidos} pedidos distribu├¡dos!`, 'sucesso');
    fecharModalDistribuicao();
    carregarPedidos();
  } catch(e) { toast('Erro ao confirmar distribui├º├úo!', 'erro'); }
}

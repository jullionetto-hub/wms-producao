...
router.put('/checkout/:id/concluir', requerAuth, async (req,res) => {
  ...
  try {
    ...
    if (ck?.numero_pedido) {
      await pool.query(`UPDATE pedidos SET status_embalagem='pendente' WHERE numero_pedido=$1`,
        [ck.numero_pedido]);
    }
...
router.put('/checkout/:id/confirmar', requerAuth, async (req,res) => {
  ...
  try {
    ...
    // Marcar pedido como pendente na embalagem ANTES de responder
    const ck2 = await db.get('SELECT numero_pedido FROM checkout WHERE id=$1',[req.params.id]);
    if (ck2?.numero_pedido) {
      await pool.query(`UPDATE pedidos SET status_embalagem='pendente' WHERE numero_pedido=$1`,
        [ck2.numero_pedido]);
    }
    res.json({mensagem:'Checkout concluido!', numero_pedido: ck2?.numero_pedido});
  } catch(e){res.status(500).json({erro:e.message});}
});
...
router.put('/embalagem/:id/confirmar', requerAuth, async (req,res) => {
  try {
    ...
    const ped = await db.get('SELECT * FROM pedidos WHERE id=$1', [req.params.id]);
    if (!ped) return res.status(404).json({erro:'Pedido nao encontrado'});
    ...
    await pool.query(
      ...
      [embalado_por, hora, req.params.id]
    );
    await pool.query(
    ...
    await registrarAuditoria(req, 'EMBALAR', 'pedido', ped.id, null, {embalado_por});
...
router.get('/embalagem/stats', requerAuth, async (req,res) => {
  try {
    ...
    const stats = await db.all(`
      ...
      GROUP BY embalado_por ORDER BY total DESC
    `, [dt]);
    const totais = await db.get(`
      ...
      FROM pedidos WHERE status='concluido' AND data_pedido=$1
    `, [dt]);
